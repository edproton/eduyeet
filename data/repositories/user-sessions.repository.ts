...
export class UserLoginsRepository {
	static async insert(data: Omit<typeof userLogins.$inferInsert, 'id' | 'issuedAt' | 'lastUsed'>) {
		const [insertedLogin] = await db.insert(userLogins).values(data).returning()
		return insertedLogin
	}

	static async update(id: string, data: Partial<Omit<typeof userLogins.$inferInsert, 'id'>>) {
